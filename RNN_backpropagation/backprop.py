...
print("\nNeuronas ocultas: {0} \nEpocas: {1} \nTaza de aprendizaje: {2} \nEjemplos: {3} \nDimensiones: {4}".format(n_hidden,epochs,alpha,m,k))
#quit()
...
print("\nPesos Entrada Oculta {0} \nPesos Oculta salida {1}".format(entrada_escondida,escondida_salida))
# Entrenamiento
print("\nInicia el Entrenamiento en 3 seg.\n")
time.sleep(3)
for e in range(epochs):
    ...
    if e % (epochs / 10 ) == 0:
        ...
        print("Gradiente entrada oculta: {0} \nGradiente oculta salida: {1}".format(gradiente_entrada_escondida,gradiente_escondida_salida))
        if ult_costo  and ult_costo < costo:
            print("\n>>> Costo de  entrenamiento: ", costo, " ADVERTENCIA -  Costo subiendo")
        else:
            print("\n>>> Costo de entrenamiento: ", costo )
        ult_costo = costo 
